package com.avit.common.http4.httpClient;
...
import com.avit.common.http4.common.HttpConfig;
import com.avit.common.http4.common.HttpMethods;
import com.avit.common.http4.common.Utils;
import com.avit.common.http4.exception.HttpProcessException;
import com.avit.common.http4.httpClient.builder.HCB;

/**
 * 基于apche http4.5实现的http工具类
 * @author hudongyu
 *@date 2017-1-13
 */
public class HttpClientUtil{
	...
	//初始化默认线程池
	private static HCB hcbpool;
	
	private static HCB sslhcbpool;
	
	static{
		try {
			client4HTTP = HCB.custom().build();
			client4HTTPS = HCB.custom().ssl().build();
			hcbpool= HCB.custom().timeout(defaultConnectionTimeout).pool(defaultMaxTotalConn, defaultMaxConnPerHost);
			sslhcbpool= HCB.custom().timeout(defaultConnectionTimeout).pool(defaultMaxTotalConn, defaultMaxConnPerHost).ssl();
